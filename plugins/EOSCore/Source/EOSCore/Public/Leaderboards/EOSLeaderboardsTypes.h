...
//		Operations
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ //
struct FQueryLeaderboardUserScoresOperation : public EOS_Leaderboards_QueryLeaderboardUserScoresOptions
{
public:
	FQueryLeaderboardUserScoresOperation(int32 numUserIds, int32 numStatInfo, int64_t startTime, int64_t endTime)
		: EOS_Leaderboards_QueryLeaderboardUserScoresOptions()
	{
		UserIdsArray.SetNumUninitialized(numUserIds);
		UserScoresArray.SetNumUninitialized(numStatInfo);

		for (int32 i = 0; i < numStatInfo; i++)
		{
			UserScoresArray[i].ApiVersion = EOS_LEADERBOARDS_USERSCORESQUERYSTATINFO_API_LATEST;
			UserScoresArray[i].StatName = new char[256];
		}

		ApiVersion = EOS_LEADERBOARDS_QUERYLEADERBOARDUSERSCORES_API_LATEST;
		UserIds = UserIdsArray.GetData();
		UserIdsCount = numUserIds;
		StatInfo = UserScoresArray.GetData();
		StatInfoCount = numStatInfo;
		StartTime = startTime;
		EndTime = endTime;
	}
	~FQueryLeaderboardUserScoresOperation()
	{
		for (int32 i = 0; i < UserScoresArray.Num(); i++)
		{
			delete[] UserScoresArray[i].StatName;
		}
	}
public:
	TArray<EOS_ProductUserId> UserIdsArray;
	TArray<EOS_Leaderboards_UserScoresQueryStatInfo> UserScoresArray;
};